...
function PatientList({ patients, onStart }) {
  return (
    <section className="doc-patient-list">
      <h2>Nutrition Queue</h2>
      {patients.length === 0 ? (
        <p className="doc-status-box">No patients are currently waiting for nutrition review.</p>
      ) : (
...
function NutritionModal({ isOpen, patient, availableDrugs, availableDrugsLoading, onLoadAvailableDrugs, onClose, onSubmit }) {
  ...
  const hasAnyContent = useMemo(
    () => Object.values(form).some((value) => value.toString().trim() !== "") || validPrescriptions.length > 0,
  ...
  const submit = (event) => {
    ...
    if (!hasAnyContent) {
      setError("The nutrition consultation form is empty.");
      return;
    }
    ...
    Promise.resolve(
      onSubmit({
        ...
        dietary_history: form.dietaryHistory,
        nutritional_assessment: form.nutritionalAssessment,
        medical_health_conditions: form.medicalHealthConditions,
        child_feeding_history: form.childFeedingHistory,
        lifestyle_assessment: form.lifestyleAssessment,
        nutrition_diagnosis: form.nutritionDiagnosis,
        risk_level: form.riskLevel,
        nutrition_plan: form.nutritionPlan,
    ...
    <div className="doc-modal-overlay" onClick={onClose}>
      <div className="doc-modal" onClick={(event) => event.stopPropagation()}>
        <div className="doc-modal-header">
          <h3>Nutrition Consultation: {patient.name}</h3>
        ...
        <form className="doc-modal-body" onSubmit={submit}>
          ...
          <div className="doc-section">
            ...
            <SectionGuide>Describe the reason for the visit such as weight loss, malnutrition, obesity, or appetite issues.</SectionGuide>
            <textarea rows={6} className="pediatric-writing-box" value={form.presentingComplaint} onChange={handleField("presentingComplaint")} />
          </div>

          <div className="doc-section">
            <h4>2. Dietary History</h4>
            <SectionGuide>Describe the usual breakfast, lunch, dinner, snacks, water intake, and any special diets.</SectionGuide>
            <textarea rows={7} className="pediatric-writing-box" value={form.dietaryHistory} onChange={handleField("dietaryHistory")} />
          </div>

          <div className="doc-section">
            <h4>3. Nutritional Assessment</h4>
            <SectionGuide>Describe appetite, recent weight gain or loss, feeding difficulties, and signs of malnutrition.</SectionGuide>
            <textarea rows={7} className="pediatric-writing-box" value={form.nutritionalAssessment} onChange={handleField("nutritionalAssessment")} />
          </div>

          <div className="doc-section">
            <h4>4. Medical and Health Conditions</h4>
            <SectionGuide>Record conditions affecting nutrition such as diabetes, hypertension, kidney disease, or gastrointestinal issues.</SectionGuide>
            <textarea rows={6} className="pediatric-writing-box" value={form.medicalHealthConditions} onChange={handleField("medicalHealthConditions")} />
          </div>

          <div className="doc-section">
            <h4>5. Infant/Child Feeding History</h4>
            <SectionGuide>Complete this if applicable: breastfeeding duration, weaning age, and type of complementary feeding.</SectionGuide>
            <textarea rows={6} className="pediatric-writing-box" value={form.childFeedingHistory} onChange={handleField("childFeedingHistory")} />
          </div>

          <div className="doc-section">
            <h4>6. Lifestyle Assessment</h4>
            <SectionGuide>Describe physical activity level, alcohol use, and any substance use.</SectionGuide>
            <textarea rows={6} className="pediatric-writing-box" value={form.lifestyleAssessment} onChange={handleField("lifestyleAssessment")} />
          </div>

          <div className="doc-section">
            <h4>7. Nutrition Diagnosis</h4>
            <SectionGuide>Select the nutrition diagnosis and risk level.</SectionGuide>
            <div className="pediatric-mini-grid pediatric-mini-grid-two">
              <label className="pediatric-field">
                <span>Nutrition diagnosis</span>
                <select value={form.nutritionDiagnosis} onChange={handleField("nutritionDiagnosis")}>
              ...
              <label className="pediatric-field">
                ...
                <select value={form.riskLevel} onChange={handleField("riskLevel")}>
          ...
          <div className="doc-section">
            <h4>8. Nutrition Plan</h4>
            <SectionGuide>Describe diet recommendations, supplements, child feeding plan where relevant, and follow-up advice.</SectionGuide>
            <textarea rows={7} className="pediatric-writing-box" value={form.nutritionPlan} onChange={handleField("nutritionPlan")} />
          </div>
          ...
          <div className="doc-actions">
            ...
            <button type="submit" className="btn-maroon" disabled={isSubmitting}>
              {isSubmitting ? "Submitting..." : "Submit Nutrition Consultation"}
            </button>
          </div>
        </form>
      </div>
...
export default function NutritionConsultationPage({ currentUser, onLogout }) {
  ...
  const handleSubmit = useCallback(async (payload) => {
    await submitNutritionConsultation(payload);
    setSuccessMessage(`Nutrition consultation submitted for ${selectedPatient?.name || "patient"}.`);
    closeModal();
    await refresh({ source: "after-nutrition-consultation" });
  }, [closeModal, refresh, selectedPatient]);

  return (
    <div className="doc-page">
      <div className="doc-container">
        <Header clinicianName={currentUser ? currentUser.username : "Nutritionist"} onLogout={onLogout} />
        ...
        {loading ? <p className="doc-status-box">Loading nutrition queue...</p> : <PatientList patients={patients} onStart={openConsultation} />}
        <NutritionModal
          isOpen={modalOpen}
